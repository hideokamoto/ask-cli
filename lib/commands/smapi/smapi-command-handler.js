...
const smapiCommandHandler = (swaggerApiOperationName, flatParamsMap, commanderToApiCustomizationMap, inputCmdObj) => {
    ...
    const refreshToken = resolveProviderChain([process.env.ASK_REFRESH_TOKEN, AppConfig.getInstance().getToken(profile).refresh_token]);
    ...
    client.apiConfiguration.apiEndpoint = smapiEndpoint;
