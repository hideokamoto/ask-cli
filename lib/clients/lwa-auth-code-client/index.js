...
module.exports = class LWAAuthCodeClient {
    ...
    refreshToken(token, callback) {
        ...
        httpClient.request(options, 'GET_ACCESS_TOKEN_USING_REFRESH_TOKEN', this.config.doDebug, (err, response) => {
            ...
            if (!stringUtils.isNonBlankString(expiresIn)) {
                return callback('Refresh lwa tokens failed, invalid response without "expires_in". Please run again with --debug for more details');
            }
